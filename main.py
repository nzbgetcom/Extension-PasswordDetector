...
def check_passwordstrings(outtext,errtext):
	if check_verbose_logging():
		if len(outtext)>0:
			print("out: " + outtext.translate(str.maketrans('','','\r')))	
		if len(errtext)>0:
			print("error: " + errtext.translate(str.maketrans('','','\r')))
	...
	for file in files:
		...
		if not "tmp" in file:
			try:
				...
				if check_passwordstrings(out.decode('utf-8'), err.decode('utf-8')):
					return True
...
def connect_to_nzbget():
	...
	nzbget = ServerProxy(xmlRpcUrl, verbose=check_verbose_logging())
	return nzbget
...
def call_nzbget_direct(url_command) -> str:
	# First we need to know connection info: host, port and password of NZBGet server.
	# NZBGet passes all configuration options to scripts as environment variables.
	host = os.environ['NZBOP_CONTROLIP']
	if host == '0.0.0.0': host = '127.0.0.1'
